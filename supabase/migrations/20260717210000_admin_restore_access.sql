-- Allow the owner console to restore a suspended member back to "active".
-- Revoked stays permanent by product decision: the function still refuses
-- any change once a member is revoked, so restore is only reachable from
-- "suspended".
...
begin
  ...
  if current_member.status = 'revoked'
     or current_member.status = p_status then
    return query select
      'conflict'::text,
