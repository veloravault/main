...
const vaultTypeMetaSource = readFileSync(new URL("../src/lib/vaultTypeMeta.ts", import.meta.url), "utf8");
...
test("GlobalSearch queries secure_credentials and maps each row to its own type-specific result", () => {
  ...
  for (const type of ["ssh_key", "crypto_wallet", "api_credential", "wifi_credential", "two_factor_backup"]) {
    assert.match(vaultTypeMetaSource, new RegExp(`${type}:\\s*\\{[^}]*icon:[^}]*label:[^}]*color:[^}]*bg:[^}]*gradient:`), `expected a VAULT_TYPE_META entry for "${type}"`);
  }
